...
//    log(JSON.stringify(currentState, null, 4));
    ...
    // strip autocrud metadata
    for (const tableName in currentState)
    {
      for (const field in currentState[tableName].schema)
      {
        const options = currentState[tableName].schema[field];
        if (!options) continue;
          
        delete options.publicType;
        delete options.detailedType;
        delete options.localName;
        delete options.def;
        delete options.nonForm;
        delete options.apiArea;
      }
    }
...
const writeMigration = function(revision, migration, migrationsDir, name = '', comment = '')
{
    ...
    _commandsUp = beautify(_commandsUp, {indent_size: 2}).replace(/  return \[{/g, '  return [\n    {');
    _commandsDown = beautify(_commandsDown, {indent_size: 2}).replace(/  return \[{/g, '  return [\n    {');
    let info = {
        revision,
        name,
        created: new Date(),
    ...
    let sequelizePackageName = 'sequelize-cockroachdb';
    try
    {
        require(sequelizePackageName);
    }
    catch
    {
        sequelizePackageName = 'sequelize';
    }
    
    let template = `'use strict';

var Sequelize = require('${sequelizePackageName}');
      ...
      return new Promise(function(resolve, reject)
      {
        function next()
        {
          if (index < commands.length)
          {
            ...
            {
              const isAsync = command.fn.constructor.name === "AsyncFunction";
              if (!isAsync) throw new Error('command fn must be async');
              command.fn(queryInterface, command.params).then(next, reject);
            }
            else
            {
              const theInterface = command.fn === 'query' ? queryInterface.sequelize : queryInterface;
              theInterface[command.fn].apply(theInterface, command.params).then(next, reject);
            }
          }
          else
            resolve();
...
`;
    ...
    name = name.replace(' ', '_');
    let filename = path.join(migrationsDir, revision.toString().padStart(7, '0') + ((name != '') ? `-${name}` : '') + '.js');
